...
pub const ProcStat = struct {
    ...
    pub fn get(allocator: std.mem.Allocator, pid: i32) !ProcStat {
        ...
        const file_data: []u8 = try file.reader().readAllAlloc(allocator, 1000);
        defer allocator.free(file_data);

        var iter = std.mem.splitAny(u8, trim(file_data), " ");

        var stat: ProcStat = std.mem.zeroInit(ProcStat, .{
            .allocator = allocator,
        });
        ...
        while (iter.next()) |val| {
            switch (i) {
                ...
                1 => stat.comm = try std.fmt.allocPrint(allocator, "{s}", .{val}),
                2 => stat.state = try std.fmt.allocPrint(allocator, "{s}", .{val}),
    ...
    pub fn deinit(self: *ProcStat) void {
